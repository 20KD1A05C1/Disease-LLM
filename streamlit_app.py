...
if prompt := st.chat_input("What symptoms are you experiencing?"):
   ...
   cypher_query = """MATCH (s:Symptom)-[:INDICATES]->(d:Disease)-[:TREATED_BY]->(m:Medicine) WHERE s.name IN ['fever', 'cold']RETURN d.name AS Disease, collect(s.name) AS Symptoms, collect(m.name) AS Medicines LIMIT 5"""
